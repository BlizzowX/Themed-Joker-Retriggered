local jokerInfo = {   
    ...
    config = {extra = {xmult_per_heart = 0.1, xmult_total = 1}},
    loc_vars = function(self, info_queue, card)
        local hearts_in_deck = 0
        if G.playing_cards then
            for _, playing_card in ipairs(G.playing_cards) do
                if playing_card:is_suit("Hearts") then
                    hearts_in_deck = hearts_in_deck + 1                    
                end
            end
        end
        card.ability.extra.xmult_total = 1 + (hearts_in_deck * card.ability.extra.xmult_per_heart)
        return { vars = { card.ability.extra.xmult_per_heart, card.ability.extra.xmult_total } }
    end,
